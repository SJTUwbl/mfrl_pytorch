...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'mfac', 'mfq', 'ac', 'dqn'}, help='choose an algorithm from the preset', required=True)
    parser.add_argument('--oppo', type=str, choices={'mfac', 'mfq', 'ac', 'dqn'}, help='indicate the opponent model')
    ...
    models = [spawn_ai(args.algo, env, handles[0], args.algo + '-me', args.max_steps), spawn_ai(args.oppo, env, handles[1], args.oppo + '-opponent', args.max_steps)]
    models[0].load(main_model_dir)
    models[1].load(oppo_model_dir)

    runner = tools.Runner(env, handles, args.map_size, args.max_steps, models, battle, render=args.render)
    win_cnt = {'main': 0, 'opponent': 0}

    for k in range(0, args.n_round):
        runner.run(1.0, k, win_cnt=win_cnt)
